...
internal class AnthropicKernelAdapter(
    ...
    override fun liveComplete(request: AdapterRequest): ProviderCallResult {
        ...
        return try {
            ...
            val body = buildString {
                append("{")
                append("\"model\":\"").append(spec.defaultModel).append("\",")
                append("\"messages\":[{\"role\":\"user\",\"content\":\"").append(NonOpenAiJson.escape(request.prompt)).append("\"}],")
                append("\"system\":\"").append(NonOpenAiJson.escape(request.context)).append("\",")
