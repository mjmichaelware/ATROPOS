...
class CommandProjection {
    ...
    fun render(): String = JsonWriter.obj(
        ...
        "commands" to JsonWriter.arr(
            HelpRegistry.commandEntries().map { entry ->
                JsonWriter.obj(
        ...
        "quickAccess" to JsonWriter.strArr(atropos.cli.input.CommandRegistry.quickAccessCommands()),
        "families" to JsonWriter.strArr(atropos.cli.input.CommandRegistry.families().sorted()),
        "sections" to JsonWriter.arr(
            HelpRegistry.commandSections().map { group ->
                JsonWriter.obj(
