...
grep -Fq 'MAX_ARTIFACT_BYTES' "$root/npm/scripts/postinstall.js"
grep -Fq 'for await (const chunk of response.body)' "$root/npm/scripts/postinstall.js"
grep -Fq 'Buffer.concat(chunks, total)' "$root/npm/scripts/postinstall.js"
