...
| FTY-01/02 resume execution owner | source-wired / partial | `src/main/kotlin/atropos/cli/FactoryCommandHandler.kt`, `src/main/kotlin/atropos/core/factory/AppFactoryRouter.kt`, `src/main/kotlin/atropos/core/factory/FactoryRunHandoff.kt`, `src/main/kotlin/atropos/core/factory/FactoryRunOrchestrator.kt` | `/factory resume <runId>` → `AppFactoryRouter.resume(runId, freeze, executeWave)` → existing `FactoryObligationLoop`/`FactoryEvidenceWaveExecutor` | Handoff now persists an attested evidence path after generation and CLI resume executes remaining waves; missing path fails actionable. Hosted Kotlin execution remains unverified. |
...
| FTY-03 / FTY-05 verification-failure re-entry | source-wired / partial | `src/main/kotlin/atropos/core/factory/FactoryRunOrchestrator.kt`, `scripts/backend-atom-contract-test.sh` | generation → existing `FactoryObligationLoop.executeUntilSettled()` → verifier failure/unresolved snapshot → existing freeze-checked `FactoryRepairExecutor` → obligation loop re-entry | Real obligation-wave failures now invoke the bounded repair callback with the unchanged acceptance freeze; missing/failed evidence records completion failure. Hosted factory execution remains unverified. |
...
| B-005/B-011 bridge MCP single owner | source-wired / partial | `src/main/kotlin/atropos/bridge/AtroposBridge.kt`, `src/main/kotlin/atropos/bridge/BridgeCommandRunner.kt`, `src/main/kotlin/atropos/cli/CommandRouter.kt` | bridge bootstrap → one `McpHostManager` → request-scoped `CommandRouter` injection plus `BridgeRoutes.mcpHost` | `/mcp` and `/doctor` command requests now share the bridge route host; no second MCP manager is constructed on the bridge command path. Hosted bridge/MCP execution remains unverified. |
...
| atom | status | files | caller | tests / notes |
| --- | --- | --- | --- | --- |
| B-PROV-002 / B-005 degraded route queue owner | source-wired / partial | `src/main/kotlin/atropos/core/provider/adapter/AdapterRouteFacade.kt`, `src/main/kotlin/atropos/core/agent/AgentQueueService.kt`, `scripts/backend-atom-contract-test.sh` | `/route` → `AdapterRouteFacade` → unavailable/queued route → existing `AgentQueueService(onboarding=...)` → queue processor/executor/cascade | Removed the remaining production fallback that constructed a fresh provider onboarding owner when routing queued an unavailable task. Hosted Kotlin route/queue execution remains unverified. |
...
| atom | status | files | caller | tests / notes |
| --- | --- | --- | --- | --- |
| in-scope backend status ledger gate | source-wired / partial | `scripts/backend-atom-contract-test.sh`, `STATUS-BACKEND.md` | GitHub compile/release/verify lanes → existing backend atom contract → append-only status ledger | The canonical contract now fails if any in-scope installer, provider, help, MCP, or open-core top-level atom is absent from the ledger. This protects evidence visibility without introducing a second registry or implementation. Hosted Kotlin/runtime execution remains unverified. |

### 2026-08-24T14:30:00Z · Agent: Codex GPT-5 · Batch: tier0-microatom-owner-crosswalk

| atom | status | files | caller | tests / notes |
| --- | --- | --- | --- | --- |
| B-MCP-CORE-a..k | source-wired / partial | `McpConfigParser.kt`, `McpHostManager.kt`, `TypedToolExecutor.kt`, `McpCommandHandler.kt`, `BridgeMcpHandler.kt`, `McpHostManagerTest.kt` | CLI/bridge → sole `McpHostManager` → parser, bounded transport, evidence mapper, lifecycle owner | Config parsing, stdio/HTTP transport, initialize/list/call, cleanup, allowlist refusal, CLI list/test, budget, evidence, and memory/localOnly policies are covered by the existing source contract and hosted selector. Hosted Kotlin execution remains unverified. |
| B-MCP-SENTRY-a..o / B-MCP-GH-a..r / B-MCP-GHA-a..h / B-MCP-GIT-a..g | source-wired / partial | `SentryApiClient.kt`, `SentryRepairCoordinator.kt`, `GitHubApiClient.kt`, `GitHubBinding.kt`, `.github/actions/atropos-verify/action.yml`, `.github/workflows/atropos-verify-example.yml`, `ShellCommandRunner.kt`, `BoundedGitWorktreeCommandRunner.kt` | `/sentry`, `/github`, reusable verify action, and `/git` commands → existing gate/secret/territory/evidence owners | Tier-0 auth, read/write/check operations, issue/frame/proposal/evidence flow, action fail-closed mapping, bounded local Git, and conflict/worktree operations have production caller mappings and static contracts. Credentialed/live/hosted execution remains unverified. |
...
### 2026-08-25T05:25:00Z · Agent: Codex GPT-5 · Batch: provider-status-route-owner
...
| B-MCP-GITLAB B-MCP-BITBUCKET B-MCP-LINEAR B-MCP-JIRA B-MCP-CONFLUENCE B-MCP-PLAYWRIGHT B-MCP-PUPPETEER B-MCP-DOCKER B-MCP-POSTGRES B-MCP-SQLITE B-MCP-REDIS B-MCP-SNYK B-MCP-SONAR B-MCP-DATADOG B-MCP-NEWRELIC B-MCP-SUPABASE B-MCP-FIREBASE B-MCP-SLACK B-MCP-DISCORD B-MCP-TEAMS B-MCP-ASANA B-MCP-CLICKUP B-MCP-NOTION B-MCP-PAGERDUTY B-MCP-OPSGENIE B-MCP-TERRAFORM B-MCP-PULUMI B-MCP-K8S B-MCP-AWS-READ B-MCP-GCP-READ B-MCP-AZURE-READ B-MCP-SEQUENTIAL-THINKING | config-wired / partial | `STATUS-BACKEND.md`, `docs/mcp-examples/optional-catalog.json`, `scripts/backend-atom-contract-test.sh` | existing generic MCP host and config catalog | Explicitly lists every optional catalog ID for CI ledger coverage; entries remain disabled until an operator allowlists a real upstream server. |

### 2026-08-25T09:00:00Z · Agent: Codex GPT-5 · Batch: adapter-and-worker-provider-owner
...
### 2026-08-25T07:05:00Z · Agent: Codex GPT-5 · Batch: bridge-ci-output-redaction

| atom | status | files | caller | tests / notes |
| --- | --- | --- | --- | --- |
| B-005 / secret-safe command and CI output | source-wired / partial | `src/main/kotlin/atropos/bridge/BridgeCommandHandler.kt`, `src/main/kotlin/atropos/cli/CiCommandHandler.kt`, `scripts/backend-atom-contract-test.sh` | `/v1/command` and `/ci run next` → existing command/queue owners → `RedactionFilter` → JSON/terminal output | Bridge command output and local CI job tails now pass through the existing redaction boundary immediately before rendering. Backend atom contract, hosted selector (`440 tests`), and diff check pass; hosted secret-fixture execution remains unverified. |
...
### 2026-08-25T13:30:00Z · Agent: Codex GPT-5 · Batch: gha-action-contract-reverification
...
| atom | status | files | caller | tests / notes |
| --- | --- | --- | --- | --- |
| B-PROV-002 / status and execution truth projection | source-wired / partial | `src/main/kotlin/atropos/core/provider/ProviderTruthService.kt`, `src/main/kotlin/atropos/cli/StatusCommandHandler.kt`, `src/main/kotlin/atropos/core/agent/AgentService.kt`, `src/main/kotlin/atropos/core/dag/DagExecutionService.kt`, `src/main/kotlin/atropos/core/autonomous/AutonomousOrchestrator.kt`, `src/test/kotlin/atropos/core/provider/ProviderTruthModelsTest.kt`, `scripts/backend-atom-contract-test.sh` | existing onboarding inventory → `ProviderTruthService.filterToHealthy()` → `/status endpoints`, AgentService, DAG execution, autonomous orchestration | Truth projections now filter ask/patch order through the persisted healthy-and-enabled set, so a provider disabled or live-test-failed in `providers.json` cannot reappear in status/execution projections. Static backend/provider/connect/MCP/GHA contracts, hosted selector (`441 tests`), orphan gate (4 historical baseline orphans), and `git diff --check` pass. Kotlin execution and GitHub-hosted CI remain unverified; no green claim. |
...
| atom | status | files | caller | tests / notes |
| --- | --- | --- | --- | --- |
| F-WEB-004/F-WEB-005 backend WIRE: workspace tree and file read/write | source-wired / partial | `src/main/kotlin/atropos/bridge/BridgeWorkspaceHandler.kt`, `src/main/kotlin/atropos/bridge/BridgeRoutes.kt`, `src/test/kotlin/atropos/bridge/BridgeWorkspaceRoutesTest.kt`, `.github/workflows/compile-gate.yml`, `scripts/atropos-verify-worktree.sh` | `AtroposBridge.server()` → `BridgeRoutes.table()` → `BridgeWorkspaceHandler.tree/file` → bounded repo-relative filesystem owner | Added `GET /v1/workspace/tree`, `GET /v1/workspace/file`, `PUT /v1/workspace/file`; reads are size-bounded/redacted, writes enforce traversal/symlink/protected-path checks, explicit durable territory/read-only assignments, and `TypedToolExecutor`/`FILE_MUTATION`. Focused happy-path and traversal/read-only tests are in the hosted selector (`442 tests`). Gradle focused run timed out at task-graph calculation with exit 124; no Kotlin or web-green claim. |
| ADD-W-005/006 backend WIRE: node progress SSE and territory membership | source-wired / partial | `src/main/kotlin/atropos/bridge/BridgeEventHub.kt`, `src/main/kotlin/atropos/bridge/BridgeEventsHandler.kt`, `src/main/kotlin/atropos/bridge/BridgeRoutes.kt`, `src/test/kotlin/atropos/bridge/BridgeStreamTest.kt` | existing `BridgeEventHub` → `BridgeEventsHandler.streamEvents` → `BridgeRoutes.streamRoutes()` → `/v1/events/stream`; workspace territory route → same `TerritoryService` store | Queue state transitions are projected as canonical `node_progress` events on the existing event hub; `/v1/events/stream` already honors session/cursor identity. Added `GET /v1/workspace/territory?path=&surface=` returning workspace membership and assignment metadata. Backend atom contract, hosted selector, and diff check pass; Kotlin execution remains unverified. |
