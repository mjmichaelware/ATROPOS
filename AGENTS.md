...
### 2026-08-03T12:27:55Z · Agent: Claude (Claude Code) · Batch: phase11-selfhost-proof-gates-001
- Paths touched: `src/main/kotlin/atropos/core/agent/SelfHostInstalledProofEvidence.kt` (+66 new), `SelfHostEvidenceBundleExporter.kt` (+9/-2), `SelfHostPromotionService.kt` (+4/-2), `scripts/selfhost-installed-proof.sh` (+5), `src/test/kotlin/atropos/core/agent/SelfHostInstalledProofEvidenceTest.kt` (+99 new), `SelfHostPromotionServiceTest.kt` (+77)
- New decoupled files: `SelfHostInstalledProofEvidence.kt`, `SelfHostInstalledProofEvidenceTest.kt`
- Atoms / phases affected: `C1-SB-02`, `C1-SB-03`; Phase 11 promotion gate enforcement and installed-proof evidence completeness
- Predicate moved: (a) `C1-SB-02` is now enforceable in test, not only in code. Every prior promotion test injected a `CompletionGateReport`, which proved the plumbing but never the gate; the new test runs the real `VerifiedCompletionGate` against a process runner whose every command exits nonzero and asserts the promotion is refused, the installed jar is unchanged, and no verified checkpoint is recorded. (b) `C1-SB-03` post-mutation `git status` evidence is now captured inside `SelfHostPromotionService` between the gate report and the swap record, so a promoted goal carries a witness naming what changed on disk. (c) A new single-responsibility owner decides whether an evidence set can support an installed-runtime claim at all, and the bundle self-reports that verdict in both Markdown and JSON.
- % delta: none claimed. Two atoms gained enforceable tests; the operator installed-runtime proof has not been rerun on this tree, so no phase percentage moves.
- Why justified: A bundle that exports cleanly is not a bundle that proves anything — an empty run and a fully proved run previously produced equally well-formed `bundle.md` files. `SelfHostInstalledProofEvidence` separates "write down what happened" from "is what happened enough", requiring a zero-exit candidate build, an independent gate report, post-mutation git status, and a completed swap. Absence is reported as typed missing parts so a refusal tells the operator which proof to rerun. A recorded `ok=false` build or `promoted=false` swap is explicitly not counted as evidence, which a marker-substring check would have accepted. The installed-proof script now fails closed when the bundle does not self-report completeness; the existing safety→director→gate→swap ordering assertion is unaffected because the status line lands between gate and swap.
- HR interrupts: none. Territory respected: no `SelfHostCradleRuntimeState*`, no calculator/App-Factory paths, no second gate/verifier/worktree system — the promotion service and exporter were extended, not replaced.
- Fingerprints: branch `phase11/selfhost-proof-gates`; manifest not refreshed (gitignored and absent from this clone)
- Verification: focused only. `SelfHostInstalledProofEvidenceTest`, `SelfHostPromotionServiceTest`, and `SelfHostEvidenceBundleExporterTest` BUILD SUCCESSFUL; `bash -n scripts/selfhost-installed-proof.sh` passes. No full suite and no JAR rebuild was run by this agent.
- New overall estimate: unchanged; Checkpoint C1 remains open pending an operator run of `scripts/selfhost-installed-proof.sh` against a freshly built JAR

### 2026-08-03T14:05:00Z · Agent: Claude (Claude Code, Director) · Batch: phase11-selfhost-full-closure-001
- Paths touched: `src/main/kotlin/atropos/core/agent/SelfHostMutationVerificationGate.kt` (+130 new), `SelfHostWorktreeNodeExecutor.kt` (+29/-4), `SelfHostWorktreeDiffInspector.kt` (+14), `SelfHostEvidenceBundleExporter.kt` (+30), `src/main/kotlin/atropos/cli/commands/SelfHostChangeRequestClassifier.kt` (+130 new), `SelfHostNaturalLanguageRouter.kt` (+47/-29 rewrite), `src/main/kotlin/atropos/core/endpoint/EndpointManifest.kt` (+88 new), `EndpointManifestValidator.kt` (+70 new), `EndpointManifestRegistry.kt` (+62 new), `src/main/kotlin/atropos/core/worktree/BoundedGitWorktreeCommandRunner.kt` (+60/-3), plus focused tests `SelfHostMutationVerificationGateTest.kt` (+200 new), `SelfHostChangeRequestClassifierTest.kt` (+120 new), `EndpointManifestValidatorTest.kt` (+130 new), `EndpointManifestRegistryTest.kt` (+115 new), `SelfHostWorktreeNodeExecutorTest.kt` (+83), `SelfHostEvidenceBundleExporterTest.kt` (+54)
- New decoupled files: `SelfHostMutationVerificationGate.kt`, `SelfHostChangeRequestClassifier.kt`, `EndpointManifest.kt`, `EndpointManifestValidator.kt`, `EndpointManifestRegistry.kt`, and four new test owners
- Atoms / phases affected: `C1-SB-02` (mutation boundary), `J009-impl` (endpoint manifest contract type + validation); Phase 11 NL entry surface
- Predicate moved: (a) **C1-SB-02 now holds at the boundary that writes.** `SelfHostWorktreeNodeExecutor` merged a mutation into the live working tree and wrote `DagNodeState.COMPLETE` directly, with `git diff --check` — a whitespace lint — as the only check in between. The compile gate was not consulted until promotion, long after the source was on disk. `SelfHostMutationVerificationGate` now puts the real `VerifiedCompletionGate` between merge and COMPLETE and reverse-applies the patch when the gate refuses; a reversal that itself fails is reported as a dirty tree rather than swallowed. `BoundedGitWorktreeCommandRunner` gained `REVERSE_APPLY_PATCH` because the worktree that produced the change is already removed by then. (b) **NL self-host routing is general.** The router matched a fixed phrase list ("build yourself", "improve atropos", "inside out"), so the ordinary form — "change the bounded process timeout in ATROPOS" — matched nothing and fell through to provider chat. `SelfHostChangeRequestClassifier` decides subject and mood independently, which is what lets the verb list widen without self-mutating on "how does ATROPOS build itself?". (c) **J009 has a real contract type.** `EndpointManifest` carries the SD2 .215 fields; the validator refuses hollow manifests and refuses retry whenever `sideEffects != NONE`, because retrying a push repeats the push.
- % delta: none claimed. Phase 11 is NOT closed and Checkpoint C1 remains open.
- Why justified: Each change closes a hole that was demonstrable in the tree, not a speculative one, and each is proved by a test that fails against the old behavior. The mutation-gate tests include a case with no injected report at all — the real `VerifiedCompletionGate` running against a process runner whose every command exits nonzero — so the refusal is the gate's, not the fixture's.
- HR interrupts: none. Director-initiated lane closures instead: W4 (worktree decoupling) was stopped mid-extraction and reverted to HEAD; W2 (route audit) stopped having written nothing; W1, W3, W4′ were stopped mid-flight at operator instruction on quota and their incomplete work was reverted. Nothing half-written was kept.
- Verification: focused only. 42 tests across `SelfHostChangeRequestClassifierTest` (8), `SelfHostNaturalLanguageRouterTest` (4), `SelfHostMutationVerificationGateTest` (6), `SelfHostWorktreeNodeExecutorTest` (5), `SelfHostEvidenceBundleExporterTest` (4), `EndpointManifestRegistryTest` (6), `EndpointManifestValidatorTest` (9) — 0 failures. `./gradlew compileKotlin` SUCCESSFUL at merge. No full suite, no JAR rebuild, no installed proof run by this agent.
- KNOWN-OPEN, explicitly not claimed (carry into the next batch):
  1. `src/main/kotlin/atropos/core/dag/DagNodeFileMutationExecutor.kt:30` writes directly to the live tree via `Files.writeString` and marks the node COMPLETE with no isolation, no compile gate, and no undo. `DagExecutionService.executeNode:187-188` routes every CREATE_FILE/EDIT_FILE node there. This is the same defect class as the one fixed in the worktree path and it is STILL OPEN.
  2. "optional add/commit/push" — the acceptance-chain link — is unimplemented. `BoundedGitWorktreeCommandRunner` now carries `STAGE_PATH`, `STAGED_PATHS`, `COMMIT_SCOPED_PATHS`, `PUSH_BRANCH` (bounded, one path per stage, `--only`-scoped commit, no `git add -A`), but no service calls them and they are UNTESTED.
  3. J009-wire/edge: no manifest is declared for any of the 18 endpoints in `StaticOperationRegistry`, so `EndpointManifestRegistry.coverage()` reports all 18 unmanifested. The machinery is proved; the catalog is absent.
  4. J010 / J011: not attempted. Director assessment (unverified by a reader) is that `DagExecutionService` already holds the deterministic DAG-supervisor responsibility J010 names and that a new `DirectorOrchestrator` would violate the non-duplication law; J011's fan-out synthesizer is App-Factory-adjacent and out of this lane.
  5. No reader verified this batch. All five were shut down at operator instruction, so `R4`'s specific question — whether the injectable `evaluateGate`/`evaluate` lambdas in `SelfHostPromotionService` and `SelfHostMutationVerificationGate` are reachable from production code — is UNANSWERED. Treat those injection points as unaudited.
- New overall estimate: unchanged. Checkpoint C1 remains open on items 1–3 above plus the operator-gated installed proof.
